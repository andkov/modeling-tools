...
# Outcomes
tableone::CreateTableOne(
  data = ds5 %>% filter(waveL==1L) # no matter which wave, person-level
  ,vars = intervention
...
ps1 <-
  twang::ps(
    ...
    ,data              = d_nia %>% filter(waveL==0) %>% as.data.frame()
...
ps1 %>% readr::write_rds(paste0("./analysis/8-model-A/twang/ps/ps1-",focal_intervention,".rds") )# off after first run
# readr::write_rds(ps1,"./analysis/8-model-A/twang/ps/ps1.rds") # off after first run
# readr::write_rds(ps1,"./analysis/6-eda-tax_year/twang/ps1_full.rds") # off after first run
# ps1 <- readr::read_rds("./analysis/8-model-A/twang/ps/ps1.rds") # to speed up report
# ps1_full <- readr::read_rds("./analysis/6-eda-tax_year/twang/ps1_full.rds") # to speed up report
...
d_nia %>% readr::write_rds(paste0("./analysis/8-model-A/twang/d/d1-",focal_intervention,".rds"))
...
ps_object <- readr::read_rds("./analysis/8-model-A/twang/ps/ps1-career_planning.rds")
d_nia <- readr::read_rds("./analysis/8-model-A/twang/d/d1-career_planning.rds") #used for group balancing

# extrac the computed weight
d <-
  ps_object$data %>%
  mutate(
  ...
  as_tibble()
# d %>% glimpse()
d     %>% filter(person_oid == sample_of_interest1) %>% select(person_oid, earnings_total, w)
d_nia %>% filter(person_oid == sample_of_interest1) %>% select(person_oid, earnings_total)

# note that d_nia_w is joined onto d_nia, which has both waves
d_nia_w <- d_nia %>% 
...
for(intervention_i in names(intervention_impact)){
  ...
  twang_object <-
    twang::ps(
      ...
      formula            = eq_formula
      ,estimand          = "ATT" # use ATT because that is consistent with ex-post net impact
      ,data              = d_nia %>% filter(waveL==0L) %>%  as.data.frame()
      # ,sampw             = d_nia$survey_weight # !!! optional
      # gradient boosting
      ,stop.method       = c("es.mean", "es.max", "ks.mean", "ks.max")
      ,n.trees           = 2000
  ...
  path_save_twang_object <- paste0("./analysis/8-model-A/twang/ps/ps-",intervention_i,".rds")
  path_save_d_nia        <- paste0("./analysis/8-model-A/twang/d/d-",intervention_i,".rds")
...
# read in PS objects
ps_path <- list.files("./analysis/8-model-A/twang/ps",full.names = T)
ls_ps <- list()
...
d_path <- list.files("./analysis/8-model-A/twang/d",full.names = T)
ls_d <- list()
for(i in seq_along(d_path)){
  path_to_d <- d_path[i]
  name_of_d <- basename(d_path[i]) %>% str_remove("^d-") %>% str_remove(".rds$")
  ls_d[[name_of_d]] <- readr::read_rds(path_to_d)
}
names(ls_d)
...
for(intervention_i in names(ls_ps)){
  ...
  stem <- "./analysis/8-model-A/twang-diagnostics/"
  ...
  ggsave(
    filename = paste0(intervention_i,"-0-group-imbalance.png")
    ,plot = g
    ,path = "./analysis/8-model-A/model-solution/"
    ,device = "png"
    ,width=12
    ,height=7
  )
...
for(intervention_i in names(ls_ps)){
  ...
  twang_object <- ls_ps[[intervention_i]]
  twang_data   <- ls_d[[intervention_i]] # used for balancing, tx = focal intervention

  ls_effect[[intervention_i ]] <- compute_effect_on_continuous(
              ps_object = twang_object
              ,outcome_name = "earnings_total"
              ,covariate_names = NULL
              ,treatment_name = "tx"
              ,out_table = "predict"
              ,d_nia_input = twang_data# tibble used in the ps model
      )
...
ds_effect %>% 
  readr::write_csv("./analysis/8-model-A/model-solution/nia-effects-null.csv")
...
ds_effect %>% 
  readr::write_csv("./analysis/8-model-A/model-solution/nia-effects-full.csv")
