...
ds1 <-
  ...
  # group_by(wave) %>% 
  #   mutate(
  #     initial_religiosity = ntile(religiosity,3)
  #     ,initial_religisity = case_when(
  #       wave == "Before" ~ initial_religiosity
  #       ,wave == "After" ~ NA
  #     )
  #   ) %>% 
  group_by(key) %>% 
  ...
  initial_religiosity <- filtered_data %>%
    filter(wave == "wave1") %>% 
    select(key, initial_religiosity = religiosity) %>%
    
    # Create a new variable called 'group' 
    mutate(religiosity_group = ntile(initial_religiosity, 3))  %>%
    mutate(religiosity_group = dplyr::recode(religiosity_group,
                                      "1" = "first_33_percentile",
                                      "2" = "second_33_percentile",
                                      "3" = "third_33_percentile"))
# Join this initial religiosity back to the original data set
  ds1 <- ds1 %>%
    left_join(initial_religiosity, by = "key") 
  ds1 %>% group_by( wave, religiosity_group)  %>% summarize(n=n(),mean = mean(religiosity))
...
f# However, this difference (.25) measures the cross-sectional change
# And we need to evaluate individual change as well

#+ graph-2-within-person-over-time -----------------------------------------
# et's plot the observed within-person change
g2a <- 
  ds1 %>% 
...
g3a <-
  ...
  geom_point(shape=21)+
  geom_smooth(aes(color=wave),method = "loess")+ # compare loess to lm
...
d_predict |> 
  ggplot(aes(x = km100_to_war, y = y_hat, group=wave, color = wave)) +
  geom_point() +
  geom_line() + 
...
g5 <- 
  ds1 %>% 
  ggplot(aes(x=waveL, y = religiosity, color = religiosity_group, group=religiosity_group))+ # note re-centering to align with model coefficients
  geom_point(alpha = .2)+
  geom_line(aes(group = key),alpha = .15)+
  # geom_smooth(method = "lm")+
  scale_x_continuous(breaks = c(0,1))+
...
# ONE continuous confounder (km100_to_war)
# Including two-way interactions
m6 <- glm( religiosity ~ wave +  km100_to_war +
             religiosity_group*wave +
              religiosity_group + religiosity_group*km100_to_war 
           # + loss_dummy3*displaced*km100_to_war # experimental
           , data = ds1 )
m6 %>% tidy() 
m6 %>% GGally::ggcoef_model()

hat_name <- "emmean" # Gaussian output from emmeans (as opposed to `fitted` from broom)
eq_emmeans <- " ~ wave * religiosity_group | km100_to_war" # but remember that intervention can't be separated from wave
# eq_emmeans <- " ~ loss_dummy3 * displaced | km100_to_war" # removing wave creates problems for ds_predict
# 
e <-
  emmeans::emmeans(
...
m6g <-
  d_predict |> 
  ggplot(aes(x = wave, y = y_hat, group = religiosity_group, color = religiosity_group, fill = religiosity_group)) +
  geom_point() +
  geom_line() + 
  # facet_wrap("km100_to_war") +
  facet_grid(. ~ km100_to_war)+
  theme_minimal()
m6g

ds1 %>% group_by(wave, religiosity_group) %>% 
  summarize(mean = mean(religiosity)) %>% 
  ggplot(aes(x=wave, y = mean,group = religiosity_group))+
