...
# ---- Define Constants ----
n_people <- 5000
client_codes <- c(11, 14, 45, 92)
code_map <- tibble(
  client_type_code = client_codes,
  pc2 = c("ETW", "ETW", "BFE", "AISH"),
  program_group = c("IS", "IS", "IS", "AISH")
)
episode_durations <- 1:36
duration_weights <- rev(episode_durations) / sum(episode_durations) # shorter episodes more likely
start_pool <- seq(as.Date("2014-01-01"), as.Date("2024-12-01"), by = "month")
max_end_date <- as.Date("2025-04-30")

# ---- Generate People and Episode Counts ----
people <- tibble(
  person_oid = 1000001:(1000000 + n_people),
  n_episodes = sample(1:5, n_people, replace = TRUE, prob = c(0.5, 0.25, 0.15, 0.07, 0.03))
)

# ---- Episode Generator Function ----
generate_episodes <- function(person_oid, n_episodes) {
  # Generate client_type_code sequence (non-decreasing)
  codes <- sample(client_codes, n_episodes, replace = TRUE) %>% sort()
  start_date <- sample(start_pool, 1)
  
  episodes <- vector("list", length = 0)
  
  for (code in codes) {
    duration <- sample(episode_durations, 1, prob = duration_weights)
    end_date <- start_date %m+% months(duration - 1)
    
    if (end_date > max_end_date) break
    
    episodes[[length(episodes) + 1]] <- tibble(
    ...
    start_date <- end_date %m+% months(1) # ensure no overlap
  }
...
ds_episode <- people %>%
  mutate(data = map2(person_oid, n_episodes, generate_episodes)) %>%
  select(data) %>%
  unnest(data) %>%
  left_join(code_map, by = "client_type_code") %>%
